# Generates a batch of zen collage wallpapers into a dedicated folder.
# Point Windows Settings > Personalization > Background > Slideshow at that folder.

param(
    [string]$WallDir   = "$HOME\Pictures\arena-scrape",
    [string]$OutputDir = "$HOME\Pictures\Wallpapers\arena-batch",
    [int]$BatchCount   = 30
)
...
Write-Host "Generating $BatchCount wallpapers into $OutputDir ..."

for ($i = 1; $i -le $BatchCount; $i++) {
    $OutputPath = Join-Path $OutputDir ("wallpaper_zen_{0:D3}.png" -f $i)
    Write-Host "  [$i/$BatchCount] $OutputPath"
    [WallpaperEngine]::CreateZenWallpaper($WallDir, $OutputPath)
}

Write-Host "Done. Point Windows Slideshow at: $OutputDir"
